...
{
    VuMeter *meter = static_cast<VuMeter*>(data);
  ...
  if (message->type == GST_MESSAGE_ELEMENT) {
    ...
    if (strcmp (name, "level") == 0) {
      ...
      for (i = 0; i < channels; ++i) {
        ...
        meter->updateRms(pow (10, rms_dB / 20));
...
main (int argc, char *argv[])
{
  ...
  g_object_set (G_OBJECT (level), "interval", 1000000000, NULL);

  bus = gst_element_get_bus (pipeline);
  watch_id = gst_bus_add_watch (bus, message_handler, static_cast<void*>(&meter));
