...
Sdp::Sdp( ) : 
     _audioPort(-1), _videoPort(0), _ip_addr( "" ), _audiocodecs(0), _videocodecs(0), _local_offer( NULL), negociator(NULL)
{
    setCodecsList();
}

Sdp::Sdp( std::string ip_addr, int aport, int vport ): 
     _audioPort( aport ), _videoPort( vport ), _ip_addr( ip_addr ), _audiocodecs(0), _videocodecs(0), _local_offer( NULL ), negociator(NULL)
{
    setCodecsList();
...
int Sdp::receivingInitialOffer( pj_pool_t* pool, pjmedia_sdp_session* remote ){
    ...
    status = pjmedia_sdp_neg_create_w_remote_offer( pool, getLocalSDPSession(), remote, &negociator );
    state = pjmedia_sdp_neg_get_state( negociator );
    printf( " STATE = %s\n" , pjmedia_sdp_neg_state_str( state ) ); 
    PJ_ASSERT_RETURN( status == PJ_SUCCESS, 1 );
...
void Sdp::toString(){
    ...
    body << "o=" << sdp->origin.user.ptr << " " << sdp->origin.id << " " << sdp->origin.id << " " << sdp->origin.net_type.ptr << " " << sdp->origin.addr_type.ptr << " " << sdp->origin.addr.ptr << "\r\n"; 
    body << "s=" << sdp->name.ptr << "\r\n";
    ...
    for( i=0;i<sdp->media[0]->desc.fmt_count;i++ ){
        printf(" %i nvlSJvbJSKLBV = %s\n" , sdp->media[0]->desc.fmt_count,sdp->media[0]->desc.fmt[i].ptr);
