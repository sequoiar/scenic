...
#        self.start_streams_tmp(caller, contact_name) DEPRECATED
    ...
    def start_streams(self, caller, contact_name):
        ...
        contact = self.get_contact(contact_name)
        self.select_contact(caller, contact_name) # DEPRECATED AND DANGEROUS ? !!!!
                ...
                try:
                    if contact.state == CONNECTED:
                        try:
                            ...
                            # self.notify(caller, "AddressBookError while trying to start streaming:" + e.message, "error")   
                            self.notify(caller, AddressBookError("AddressBookError while trying to start streaming:" + e.message))
